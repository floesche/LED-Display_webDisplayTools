...
**Status:** the five-pattern stimulus set and the short/full YAML protocols are
ready for instructor-led runs. Cue A is always reinforced, the default
conditional LED level is 25%, and the default activation stock is **hot cell**.
...
**Files:** `p3_conditioning_closedloop_short.yaml` (≈8 min) and
...
| Version | Link |
| --- | --- |
| Short | [Open P3 short](https://reiserlab.github.io/webDisplayTools/arena_studio.html?repo=reiserlab/cshl-2026-course&p=protocols/shared/p3_conditioning_closedloop_short.yaml&rig=cshl_g6_2x10_ball&advanced=0) |
| Full | [Open P3 full](https://reiserlab.github.io/webDisplayTools/arena_studio.html?repo=reiserlab/cshl-2026-course&p=protocols/shared/p3_conditioning_closedloop_full.yaml&rig=cshl_g6_2x10_ball&advanced=0) |
...
Every phase is split into 20-second closed-loop trials. At each boundary the
runner immediately reinitializes the panorama, alternating `frame_index: 25`
(cue A in front) and `frame_index: 75` (cue B in front), then starts closed loop
again. There is no gray or rest interval between trials.
...
The next trial begins immediately with the other cue initialized in front. This
prevents one uninterrupted multi-minute visual state and gives a stuck fly a
fresh visual initialization without changing the total stimulation time. It
does not physically move the fly, so immobility remains an analysis/QC measure.
...
Total: **24 trials × 20 s = 480 s = 8 min**, plus negligible command overhead.
...
Total: **54 trials × 20 s = 1,080 s = 18 min**, plus negligible command
overhead. Baseline and final probe can still be analyzed as two 2-min bins.
...
---
*Updated 2026-07-10 01:47 ET. Pattern source: `patterns/p3_conditioning/`.*
